...
function garantirCacheBase_() {
  ...
  try {
    ...
    const pessoasBuckets=Array.from({length:CACHE_BUCKETS},()=>({})),qrBuckets=Array.from({length:CACHE_BUCKETS},()=>({})),crachaPorPessoa={};
    const inscricoes=lerTabela_(CONFIG.SHEETS.INSCRICOES).rows;
    inscricoes.forEach(function(i){const id=texto_(i.ID_PESSOA);if(id){crachaPorPessoa[id]=crachaPorPessoa[id]||[];crachaPorPessoa[id].push(texto_(i.NOME_CRACHA_ORIGEM));}const numero=normalizarNumeroInscricao_(i.NUMERO_INSCRICAO);if(numero)qrBuckets[bucketCache_(numero)][numero]={idPessoa:id,categoria:texto_(i.CATEGORIA)};});
    lerTabela_(CONFIG.SHEETS.PARTICIPANTES).rows.forEach(function(p){const id=texto_(p.ID_PESSOA);if(!id)return;const nome=texto_(p.NOME),nomeCracha=nomeCrachaParaCache_(texto_(p.NOME_CRACHA),crachaPorPessoa[id]||[]);pessoasBuckets[bucketCache_(id)][id]={idPessoa:id,nome:nome,nomeCracha:nomeCracha,nomeExibicao:nomeCracha||nome,qtdInscricoes:Number(p.QTD_INSCRICOES)||0,busca:normalizarComparacao_([nome,p.NOME_NORMALIZADO,nomeCracha].join(' '))};});
...
function nomeCrachaParaCache_(canonico,valoresOrigem) { return texto_(canonico)||valorHumanoUnico_(valoresOrigem,normalizarComparacao_).valor; }
