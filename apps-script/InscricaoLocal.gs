...
function garantirCamposFormularioLocal_(form,categorias) {
  ...
  if(itemCategoria){if(itemCategoria.getType()!==FormApp.ItemType.CHECKBOX)throw criarErro_('CAMPO_FORMULARIO_INCOMPATIVEL','O campo "Categoria" deve ser do tipo Checkbox.');categoria=itemCategoria.asCheckboxItem();}
  else {categoria=form.addCheckboxItem().setTitle('Categoria');categoria.setChoiceValues(categorias);}
...
function extrairRegistroFormulario_(response) {
  ...
  return{responseId:responseId,timestamp:Utilities.formatDate(timestamp,CONFIG.TIMEZONE,"yyyy-MM-dd'T'HH:mm:ssXXX"),nome:texto_(respostas[ids.nome]),nomeCracha:texto_(respostas[ids.nomeCracha]),cpf:somenteDigitos_(respostas[ids.cpf]),email:normalizarEmail_(respostas[ids.email]),telefone:somenteDigitos_(respostas[ids.telefone]),categoria:respostas[ids.categoria],dataInscricao:Utilities.formatDate(timestamp,CONFIG.TIMEZONE,'dd/MM/yyyy'),horaInscricao:Utilities.formatDate(timestamp,CONFIG.TIMEZONE,'HH:mm:ss')};
}

function opcoesCategoriaFormulario_() { const formId=texto_(PropertiesService.getScriptProperties().getProperty(FORM_LOCAL.PROPERTIES.ID));if(!formId)throw criarErro_('FORMULARIO_NAO_CONFIGURADO','FORM_ID não está configurado.');const item=FormApp.openById(formId).getItems().find(function(campo){return texto_(campo.getTitle())==='Categoria';});if(!item||item.getType()!==FormApp.ItemType.CHECKBOX)throw criarErro_('CATEGORIA_FORMULARIO_INCOMPATIVEL','O campo Categoria do formulário deve ser do tipo Checkbox.');const opcoes=item.asCheckboxItem().getChoices().map(function(escolha){return texto_(escolha.getValue());}).filter(Boolean);if(!opcoes.length)throw criarErro_('CATEGORIAS_AUSENTES','O campo Categoria não possui opções.');return opcoes; }
function categoriasFormularioCanonicas_(valor) { const opcoes=opcoesCategoriaFormulario_(),entradas=Array.isArray(valor)?valor:[valor],selecionadas={};entradas.forEach(function(entrada){const bruto=texto_(entrada);if(!bruto)return;const opcao=opcoes.find(function(item){return normalizarComparacao_(item)===normalizarComparacao_(bruto);});if(!opcao)throw criarErro_('CATEGORIA_INVALIDA','Categoria inválida.');selecionadas[normalizarComparacao_(opcao)]=true;});const canonicas=opcoes.filter(function(opcao){return selecionadas[normalizarComparacao_(opcao)];});if(!canonicas.length)throw criarErro_('CATEGORIA_INVALIDA','Categoria inválida.');return canonicas; }
function validarRegistroFormulario_(registro) { if(!registro||!texto_(registro.responseId))throw criarErro_('FORM_RESPONSE_ID_AUSENTE','A resposta não possui identificador.');if(!texto_(registro.nome))throw criarErro_('NOME_OBRIGATORIO','Informe o nome completo.');if(registro.email&&!emailValido_(registro.email))throw criarErro_('EMAIL_INVALIDO','E-mail inválido.');registro.categoria=categoriasFormularioCanonicas_(registro.categoria).join(' | '); }
function cpfConfiavelFormulario_(valor) { const cpf=somenteDigitos_(valor);return!!cpf&&cpfValido_(cpf); }
function localizarPessoaFormulario_(registro,pessoas,numero) { const porNumero=pessoas.filter(function(p){return numerosInscricaoUnicos_(texto_(p.NUMEROS_INSCRICAO).split(/[|,;\s]+/)).indexOf(numero)!==-1;});if(porNumero.length===1)return porNumero[0];const cpf=cpfConfiavelFormulario_(registro.cpf)?somenteDigitos_(registro.cpf):'',porCpf=cpf?pessoas.filter(function(p){return somenteDigitos_(p.CPF)===cpf;}):[];if(porCpf.length>1)throw criarErro_('CPF_AMBIGUO','CPF associado a mais de uma pessoa.');if(porCpf.length===1)return porCpf[0];const porEmail=registro.email?pessoas.filter(function(p){return normalizarEmail_(p.EMAIL)===registro.email;}):[];return porEmail.length===1?porEmail[0]:null; }
...
function reprocessarInscricoesLocalPendentes() { const formId=texto_(PropertiesService.getScriptProperties().getProperty(FORM_LOCAL.PROPERTIES.ID));if(!formId)throw criarErro_('FORMULARIO_NAO_CONFIGURADO','Execute criarFormularioInscricaoLocal primeiro.');const controle=lerTabela_(CONFIG.SHEETS.INSCRICOES_LOCAL_CONTROLE),concluidos={};controle.rows.forEach(function(r){if(['PROCESSADA','ASSOCIADA','DUPLICADA'].indexOf(normalizarComparacao_(r.STATUS))!==-1)concluidos[texto_(r.RESPONSE_ID)]=true;});const resumo={analisadas:0,reprocessadas:0,erros:0,errosDetalhes:[]};FormApp.openById(formId).getResponses().forEach(function(response){const id=texto_(response.getId());if(!id||concluidos[id])return;resumo.analisadas++;try{processarInscricaoLocal_(extrairRegistroFormulario_(response));resumo.reprocessadas++;}catch(erro){resumo.erros++;resumo.errosDetalhes.push({codigo:texto_(erro&&erro.code)||'ERRO_DESCONHECIDO',mensagemTecnica:texto_(erro&&erro.message).slice(0,180)});}});console.log(JSON.stringify(resumo,null,2));return resumo; }
