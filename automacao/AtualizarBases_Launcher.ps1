...
$job=Start-Job -ScriptBlock { & 'C:\UNIMED_EXAMES_ABERTOS\AtualizarBases_Simposio.ps1' }
$timer.Add_Tick({ if($job.State -in @('Completed','Failed','Stopped')){ $timer.Stop(); $o=Receive-Job $job -ErrorAction SilentlyContinue | Select-Object -Last 1; if($null -eq $o){ $o=[pscustomobject]@{status='ERRO';codigo='LAUNCHER_RESULT_ERROR'} }; if($o.status -eq 'CONCLUIDA'){ $label.Text='ATUALIZAÇÃO CONCLUÍDA'+[Environment]::NewLine+'Base: versão '+$o.baseVersion } elseif($o.status -eq 'ATUALIZADA'){ $label.Text='BASE JÁ ATUALIZADA'+[Environment]::NewLine+'Nenhum novo arquivo Event3 foi encontrado.' } else { $codigo=if($o.codigo){$o.codigo}else{'LAUNCHER_RESULT_ERROR'}; $label.Text='NÃO FOI POSSÍVEL ATUALIZAR'+[Environment]::NewLine+'Código: '+$codigo+[Environment]::NewLine+'A base atual foi preservada.'; try{Add-Content -LiteralPath 'C:\UNIMED_EXAMES_ABERTOS\logs\AutoImportacao_Simposio.log' -Encoding UTF8 -Value ('{0} | ERRO | codigo={1}' -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'),$codigo)}catch{} }; $close.Enabled=$true; Remove-Job $job -Force } })
